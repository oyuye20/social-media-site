...
import {CommentTypes, Users_liked} from '../types/user';
...
import {FetchPostsQuery} from "@/apiCall/TanStackQuery/postQuery.tsx";
import { PostMutation } from "@/apiCall/TanStackQuery/postQuery.tsx";
import {getPosts} from "@/apiCall/postRequests..ts";

const NewsFeed = (() => {
    const posts = FetchPostsQuery();
    const getUser = GetUserQuery();
    const dispatch = useDispatch();
    const { likePostMutation } = PostMutation();
                                    ...
                                    <div className="flex flex-col">
                                        <HoverCard>
                                            <HoverCardTrigger asChild>
                                                <Link to={`/profile/${post.post_author_id}`} className='text-white font-bold text-lg hover:underline
                                                underline-offset-4 cursor-pointer'>{post.post_author}</Link>
                                            </HoverCardTrigger>
