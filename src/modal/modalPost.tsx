...
getUserInfo, likePost, 
getCommentsByID} from "../utils/axiosRequest";
    ...
    if(fetchPostID.isFetched){
        return(
            <>
                ...
                min-h-dvh absolute z-20 grid">
                    <div className={`grid h-full rounded-xl 
                    ${fetchPostID.data?.image ? 
                    `grid-cols-3` : `grid-cols-1`}`}>

                        {/* IMAGE SECTION AND HIDE THIS IF NO IMAGE ATTACHED */}
                        {fetchPostID.data?.image != null && (
                            <div className="col-span-2 flex 
                            items-center p-5 justify-center bg-[#1C273D]">
                        ...
                        <div className="flex flex-col gap-3 p-3 relative  bg-[#1C273D]">
                            ...
                            <div className="NAME flex p-3 gap-3">        
                                ...
                                <div className="flex flex-col px-2">
                                    ...
                                    {fetchPostID.data?.post_author}</span>

                                    <span className='text-white text-md 
                                    font-bold'>10 hours ago</span>
                                </div>
                                    ...
                                    <div className="relative">
                                        ...
                                        {dropdown && (
                                            ...
                                            rounded-lg gap-4">
                                                ...
                                                <button onClick={()=> onDeletePost(id!)} 
                            ...
                            <div className="CAPTION flex p-3 gap-3">
                                <span className="span font-medium text-white">
                                    {fetchPostID.data?.post_description}
                                </span>
                            </div>
                            ...
                            <div className="flex gap-3 p-3">
                                <div className="flex items-center gap-1">
                                    <button type="button" onClick={()=> likePosts(id!)}  
                                ...
                                <div className="flex items-center">
                                    ...
                                    <span className='font-bold text-white'>{fetchPostID.data?.total_comments}</span>                     
                                </div>                                 
                            </div>
                            ...
                            border-[#0C1629] border-y-2 overflow-y-scroll h-[500px]">
                                ...
                                {commentIDQuery.data?.length === 0 ? (
                                    <>
                                    <h1 className="text-white text-xl">Empty Comments, Create a new One</h1>
                                    </>
                                ):(
                                    <>
                                    {commentIDQuery.data?.map((c, index) => {
                                        return (
                                            <div className="flex flex-col gap-2"  key={index}>
                                                ...
                                                <div className="flex gap-2">
                                                    ...
                                                    <div className="bg-[#111d2f] rounded-lg p-3 
                                                    flex flex-col  gap-3 w-full"> 
                                                        <span className='font-bold text-xl text-white'>{c.commenter}</span>
                                                        <div className="span font-medium text-white">{c.comment}</div>                                  
                                                    </div>
                                                </div>
                                                
                                                {/* <div className="timestamp">
                                                    <span className='text-md text-white'>{c.comment_date_posted.toString()}</span>
                                                </div>  */}                       
                                            </div> 
                                        )
                                    })}
                            ...
                            <div className="comments flex gap-3 w-full">
                                ...
                                <form onSubmit={handleSubmit(onSubmit)} className="flex relative items-center w-full justify-end">
                                    <input type="hidden" {...register("post_id")} value={id} />
